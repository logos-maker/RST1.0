...
typedef struct midi_events{
	int32_t number_of_events;	// number of plugMidiEvent's in MidiEvent[2] (can be set to zero if a note_id_end it set).
	plug_v note_id;			// Is set to zero, if not used or supported. note_id is generated by the DAW.
	plug_v note_id_end;		// Is set to zero, if not used or supported. killed voice_id. Sent by plug to host when synthesizer voice has ended and no longer generates sound.
